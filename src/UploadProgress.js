  ...
  return (
    <div className='ma4 flex-auto flex items-center'>
      <div className='w-100 pa4 ba bg-white'>
        <p className='f4 mv1'><strong>{fmt(totalFiles)}</strong> files, {storedBytes ? <span><strong>{bytes(storedBytes)}</strong><span> of </span></span> : ''}<strong>a lot</strong>{storedChunks ? <span className='f6 fr mt1'>{fmt(storedChunks)} chunk{storedChunks > 1 ? 's' : ''}</span> : ''}</p>
        {storedBytes
